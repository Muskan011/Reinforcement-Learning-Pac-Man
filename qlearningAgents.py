...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(lactions) == 0:
          return 0.0
        ret = max([self.getQValue(state, action) for action in lactions])
        return ret
        util.raiseNotDefined()
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(lactions) == 0:
          return None
        for actions in lactions:
          q_value = self.q_value[(state, actions)]
          if q_value > maxvalue:
                maxvalue, action = q_value, actions
        return action
        util.raiseNotDefined()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for k in feat:
          self.weights[k] += self.alpha * diff * feat[k]
